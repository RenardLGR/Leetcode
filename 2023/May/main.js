...
var findSubstring = function(s, words) {
    //build every combinations which are all the same length l
    //for each letters in s, check if the following letters of length l is a combination
    ...
    for(let i=0 ; i<=s.length-l ; i++){
        let subString = s.slice(i, i+l)
        if(combinations.includes(subString)){
            res.push(i)
        }
    }
    ...
    //build every combinations
    function combinator(inProgress, remaining){
        if(remaining.length === 0){
            combinations.push(inProgress.join(''))
            return
        }
        ...
        for(let i=0 ; i<remaining.length ; i++){
            ...
            combinator([...inProgress, remaining[i]], newRemaining)
        }
    }
};
